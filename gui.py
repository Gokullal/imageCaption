...
openai.api_key = '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'


def send_to_chatgpt(caption):
...
def classify(file_path):
    ...
    # Send the captions to ChatGPT
    chatgpt_greedy = send_to_chatgpt(pred)
    chatgpt_beam_3 = send_to_chatgpt(beam_3)
    chatgpt_beam_5 = send_to_chatgpt(beam_5)
  # Define labels to display ChatGPT responses
    chatgpt_greedy_label = Label(top, background='#CDCDCD', font=('arial', 15))
    chatgpt_beam_3_label = Label(top, background='#CDCDCD', font=('arial', 15))
    chatgpt_beam_5_label = Label(top, background='#CDCDCD', font=('arial', 15))

    # Display the ChatGPT responses in the GUI
    chatgpt_greedy_label.configure(
        foreground='#000', text='ChatGPT Greedy: ' + chatgpt_greedy)
    chatgpt_greedy_label.pack(side=BOTTOM, expand=True)
    chatgpt_beam_3_label.configure(
        foreground='#000', text='ChatGPT Beam_3: ' + chatgpt_beam_3)
    chatgpt_beam_3_label.pack(side=BOTTOM, expand=True)
    chatgpt_beam_5_label.configure(
        foreground='#000', text='ChatGPT Beam_5: ' + chatgpt_beam_5)
    chatgpt_beam_5_label.pack(side=BOTTOM, expand=True)


def show_classify_button(file_path):
    classify_b = Button(top, text="Generate",
...
# label2.pack(side = BOTTOM, expand = True)
heading = Label(top, text="Caption Generator (Flickr30k)",
                pady=20, font=('arial', 22, 'bold'))
